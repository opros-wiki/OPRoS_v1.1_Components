...
{
	if(handle == NULL) {
		PrintMessage("ERROR : SerialCommunication::Enable() -> The handle of API is not created.\n");
		return API_ERROR;
	}

	if(handle->OpenSerial() < 0) {
		PrintMessage("ERROR : SerialCommunication::Enable() -> Can't open serial\n");
		return API_ERROR;
	}
...
{
	if(handle == NULL) {
		PrintMessage("ERROR : SerialCommunication::Write() -> The handle of API is not created.\n");
		return API_ERROR;
	}
...
{
	if(handle == NULL) {
		PrintMessage("ERROR : SerialCommunication::Read() -> The handle of API is not created.\n");
		return API_ERROR;
	}
