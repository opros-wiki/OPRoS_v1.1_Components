#include "StellaB2WheelController.h"
...
#define DEVICE_CREATED 0
#define DEVICE_READY   1
#define DEVICE_ACTIVE  2
#define DEVICE_ERROR   3

StellaB2WheelController::StellaB2WheelController() : m_uart(NULL), m_stellaB1(NULL)
{
}

StellaB2WheelController::~StellaB2WheelController()
{
	Finalize();
}

int32_t StellaB2WheelController::Initialize(OPRoS::Property parameter)	
{
	...
	if (((SerialCommunication *)m_uart)->InitalizeSerial(parameter) != API_SUCCESS)
	{
...
int32_t StellaB2WheelController::Finalize()
{
	if (status == DEVICE_ACTIVE || status == DEVICE_ERROR)
		Disable();
...
int32_t StellaB2WheelController::Enable()
{
	...
	if (((SerialCommunication *)m_uart)->EnableSerial() != API_SUCCESS)
	{
...
int32_t StellaB2WheelController::Disable()
{
	...
	else if(status != DEVICE_ACTIVE && status != DEVICE_ERROR)
	{
		PrintMessage(DEBUG_MESSAGE("Precondition not met").c_str());
		m_rwLockStatus.WriterUnlock();
		return API_ERROR;
	...
	if (((SerialCommunication *)m_uart)->DisableSerial() != API_SUCCESS)
	{
...
int32_t StellaB2WheelController::SetProperty(OPRoS::Property parameter )
{
	if (status == DEVICE_CREATED)
	{
		PrintMessage(DEBUG_MESSAGE("Precondition not met").c_str());
	...
	if (((SerialCommunication *)m_uart)->SetParameterSerial(parameter) != API_SUCCESS)
	{
...
int32_t StellaB2WheelController::GetProperty(OPRoS::Property &parameter )
{
	if (status == DEVICE_CREATED)
	{
		PrintMessage(DEBUG_MESSAGE("Precondition not met").c_str());
	...
	if(((SerialCommunication *)m_uart)->GetParameterSerial(parameter) != API_SUCCESS)
	{
...
/*
int StellaB2WheelController::OnExecute()
{
...
*/
bool StellaB2WheelController::ProcessOdometricLocalization()
{
...
int32_t StellaB2WheelController::SetPosition(OPRoS::MobilePositionData position )
{
	m_rwLockStatus.ReaderLock();
	if (status != DEVICE_ACTIVE)
	{
...
int32_t StellaB2WheelController::GetPosition(OPRoS::MobilePositionData& position )
{
	m_rwLockStatus.ReaderLock();
	if (status != DEVICE_ACTIVE)
	{
...
int32_t StellaB2WheelController::GetOdometry(std::vector<OPRoS::Int32> &odometery)
{
	m_rwLockStatus.ReaderLock();
	if (status != DEVICE_ACTIVE)
	{
...
int StellaB2WheelController::RotateWheel( double angle, double angularVelocity )
{
	m_rwLockStatus.ReaderLock();
	if (status != DEVICE_ACTIVE)
	{
...
int StellaB2WheelController::StopWheel()
{
	m_rwLockStatus.ReaderLock();
	if (status != DEVICE_ACTIVE)
	{
...
int StellaB2WheelController::IsWheelRunning( bool& isWheelRunning )
{	
	m_rwLockStatus.ReaderLock();
	if (status != DEVICE_ACTIVE)
	{
...
//int StellaB2WheelController::DriveWheel( double linearVelocity, double angularVelocity )
//{

//}

int32_t StellaB2WheelController::SetVelocity(OPRoS::MobileVelocityData mobileVelocity)
{
	status = DEVICE_ACTIVE;
	m_rwLockStatus.ReaderLock();
	if (status != DEVICE_ACTIVE)
	...
	linearVelocity = mobileVelocity.x;
	angularVelocity = mobileVelocity.theta;
	...
	std::cout << "   speed R = " << rightWheelSpeed << "::";

	std::cout << "   speed L = " << leftWheelSpeed   << "::";
...
bool StellaB2WheelController::SetStellaB1Profile(OPRoS::Property parameter )
{
	PrintMessage("--------------------------------\r\n");
	PrintMessage("StellaB2WheelController Profiles\r\n");
	PrintMessage("--------------------------------\r\n");
...
OprosDevice *GetAPI()
{
	return new StellaB2WheelController();
}
#else
extern "C"
{
