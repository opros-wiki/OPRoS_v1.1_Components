...
int SerialCommunication::InitalizeSerial(OPRoS::Property parameter)
{
...
int SerialCommunication::FinalizeSerial(void)
{
	if(handle != NULL) {
		handle->CloseSerial();
		delete handle;
...
int SerialCommunication::EnableSerial(void)
{
	if(handle == NULL) {
		return API_ERROR;
	}
...
int SerialCommunication::DisableSerial(void)
{
	if(handle == NULL) {
		return API_ERROR;
	}
