...
bool StellaB1::WriteCommand(const std::vector<OPRoS::Char> data, int size)
{
	if (m_uart == NULL)
		return false;
...
//				size - totalSendSize)) <= 0)
		...
		if ((sendSize = m_uart->Write((std::vector<OPRoS::Char>) (data + totalSendSize)) <= 0)
		{
			errorCount++;
			continue;
		}
...
//		if (m_uart->Read(&tempReceiveData, 1) <= 0)
		...
		OPRoS::Char tempReceiveData = 0;
		if (m_uart->Read(&tempReceiveData) <= 0)
		{
			errocCount++;
			continue;
		}
