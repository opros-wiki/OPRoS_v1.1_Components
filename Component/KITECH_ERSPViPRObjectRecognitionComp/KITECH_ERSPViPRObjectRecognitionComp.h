...
/**
 *	<table><tr><td>
 *	KITECH_ERSPViPRObjectRecognitionComp는 영상을 입력받아 물체를 인식하는 컴포넌트이다.
 *	KITECH_ERSPViPRObjectRecognitionComp에는 KITECH_ERSPViPRObjectRecognitionComp.xml,VisionRecognitionServiceProvided.h가
 *	포함되어야 하며, KITECH_ERSPViPRObjectRecognitionComp.xml는 KITECH_ERSPViPRObjectRecognitionComp의 프로파일이 저장되어 있다.
...
protected:
	...
	/**
	 *	KITECH_ERSPViPRObjectRecognitionComp의 프로파일정보를 저장하는 변수이다.<br>
	 *	<!-- [표] Parameter -->
	 *	<table border=1>
	 ...
	 *	<tr><td><b>dbName</b></td> <td>std::string</td> <td>컴포넌트에서 사용할 DataBase파일로서 오브젝트의 정보가 포함되어 있다.</td></tr>
	 *	</table>
	*/
	Property parameter;
...
public:
	/**
	 *	KITECH_ERSPViPRObjectRecognitionComp 클래스의 기본 생성자이다.<br>
	 *	포트를 초기화시키는 함수를 호출한다.
	*/
	KITECH_ERSPViPRObjectRecognitionComp();

	/**
	 *	KITECH_ERSPViPRObjectRecognitionComp 클래스의 전달인자를 포함한 생성자이다.<br>
	 *	포트를 초기화시키는 함수를 호출한다.
	...
	*/
	...
	/**
	 *	KITECH_ERSPViPRObjectRecognitionComp 클래스의 기본 소멸자이다.
	*/
	virtual ~KITECH_ERSPViPRObjectRecognitionComp();

	/**
	 *	KITECH_ERSPViPRObjectRecognitionComp에서 제공하는 서비스 포트를 초기화하고 등록한다.
	*/
...
public:
	...
	// 서비스포트
	/**
	 *	KITECH_ERSPViPRObjectRecognitionComp의 프로파일을 변경한다.
	...
	/**
	 *	KITECH_ERSPViPRObjectRecognitionComp의 프로파일을 다른 컴포넌트에서 가져갈 때 사용된다.
