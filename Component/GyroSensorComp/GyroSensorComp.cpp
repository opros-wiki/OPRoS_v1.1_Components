...
ReturnType GyroSensorComp::SetParameter(Property parameter)
{
	if(gyroSensor == NULL) {
		PrintMessage("ERROR : GyroSensorComp::SetParameter() -> The handle of API is not created.\n");
...
Property GyroSensorComp::GetParameter()
{
	...
	if(gyroSensor == NULL) {
		PrintMessage("ERROR : GyroSensorComp::GetParameter() -> The handle of API is not created.\n");
...
vector<OPRoS::Rotation> GyroSensorComp::GetSensorValue()
{
	vector<OPRoS::Rotation> sensorValue;
	int ret;
	...
	lock.Lock();
	ret = gyroSensor->GetSensorValue(sensorValue);
	lock.Unlock();
...
void GyroSensorComp::portSetup() 
{
	...
	addPort("gyroSensorData", &gyroSensorData);
}
...
{
	if(gyroSensor == NULL) {
		PrintMessage("ERROR : GyroSensorComp::onStart() -> The handle of API is not created.\n");
		return lastError = OPROS_PRECONDITION_NOT_MET;
	}

	if(gyroSensor->Enable() == API_ERROR) {
		return lastError = OPROS_ENABLE_API_ERROR;
	}
...
{
	if(gyroSensor == NULL) {
		PrintMessage("ERROR : GyroSensorComp::onStop() -> The handle of API is not created.\n");
		return lastError = OPROS_PRECONDITION_NOT_MET;
	}
...
ReturnType GyroSensorComp::onExecute()
{
	...
	lock.Lock();
	if(gyroSensor->GetSensorValue(sensorValue) < 0) {
		lastError = OPROS_CALL_API_ERROR;
	}
	else {
